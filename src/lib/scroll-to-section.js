...
function isSectionVisibleInViewport(rect, offset) {
    return rect.bottom > offset + 40 && rect.top < window.innerHeight - 40;
}
function shouldSkipScroll(rect, offset) {
    if (!isSectionVisibleInViewport(rect, offset))
        return false;
    if (rect.top >= offset - 12)
        return true;
...
export function scrollToSection(sectionId) {
    ...
    if (shouldSkipScroll(rect, offset))
        return;
