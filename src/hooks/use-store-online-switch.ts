...
export function useStoreOnlineSwitch(onStoreOnlineChange?: (online: boolean) => void) {
  ...
  const requestSwitch = (online: boolean) => {
  ...
  const confirmSwitch = async (password: string) => {
    setLoading(true);
    setError("");
    try {
