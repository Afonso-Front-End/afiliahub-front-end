...
import { mergeProdutosSearch } from "@/lib/produtos-search";
import { useProdutosFilters } from "@/hooks/use-produtos-filters";
import { useSiteContent } from "@/context/site-content-context";
export function useProdutosPage(urlSearch) {
    ...
    const categoryOptions = useMemo(() => getProductCategoryOptions(content.categories.items, products), [content.categories.items, products]);
    ...
    const activeCategoryName = categoryOptions.find((entry) => entry.slug === filters.categoria)?.name ?? filters.categoria;
    return {
        header,
        storeFilters,
        ...filters,
