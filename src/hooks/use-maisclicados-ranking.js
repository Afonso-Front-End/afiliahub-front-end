...
export function useMaisClicadosRanking(cmsProducts, clickSummary) {
    const [filtroLoja, setFiltroLoja] = useState("Todos");
    const clickMap = useMemo(() => new Map(clickSummary.items.map((item) => [item.productId, item.clicks])), [clickSummary.items]);
    ...
    const lojas = useMemo(() => ["Todos", ...Array.from(new Set(ranking.map((entry) => entry.product.store)))], [ranking]);
    const filtrado = useMemo(() => filtroLoja === "Todos"
        ? ranking
        : ranking.filter((entry) => entry.product.store === filtroLoja), [filtroLoja, ranking]);
